...
### ⚠️ نکات مهم پیکربندی
- **SMS Panel Password**: رمز عبور پنل ملی‌پیامک دقیقاً `B96$1` است. در هنگام تنظیم دستی در فایل `.env` باید دقت شود که کاراکتر `$` به درستی توسط سیستم (داکر یا لینوکس) خوانده شود.
- **Docker Env Escaping**: تجربه نشان داد که در این پروژه خاص، باید از مقدار لیتراال استفاده شود.
